...
function Guess({ breed, origin, imageLink, randomDog }) {
  ...
  const isCorrectOrigin = randomDog.origin.toLowerCase() === origin.toLowerCase();
  const isPartialCorrectOrigin = randomDog.origin.toLowerCase().includes(origin.toLowerCase());

  const breedTextClass = isCorrectBreed ? 'text-green-500' : 'text-red-500';
  const originTextClass =
    isCorrectOrigin ? 'text-green-500' : isPartialCorrectOrigin ? 'text-yellow-500' : 'text-red-500';
  ...
  return (
    <div className='p-3 flex items-center justify-center cursor-pointer'>
      ...
      <div className='min-h-[20%] border-2 font-souls w-2/12 text-2xl font-semibold'>
